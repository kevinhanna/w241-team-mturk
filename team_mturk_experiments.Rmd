...
format_table(d[order(dataset_no), .("Mean Score"=mean(bounding_box_score, na.rm=T), 
      "In Treatment"=sum(in_treatment==1), 
      "In Control"=sum(in_treatment==0),
      "Total"=sum(in_treatment %in% c(0,1)),
      "Has is_mobile"=sum(is_mobile %in% c(0,1)),
      "Reward Amount"=max(Reward), # Each only has one
      "Standard Deviation"=sd(bounding_box_score, na.rm=T)
      ), 
  by=dataset_no]
)
...
worker_mean_score <- d[dataset_no==1, .(mean_worker_score = mean(bounding_box_score), in_treatment = as.integer(median(in_treatment))), keyby=WorkerId]
...
#e2_mod_2 <- d[dataset_no==2, lm(bounding_box_score ~ in_treatment+WorkTimeInSeconds)]
...
e3_mod_2 <- d[dataset_no==3, lm(bounding_box_score ~ in_treatment+as.factor(monitor))]
e3_mod_3 <- d[dataset_no==3, lm(bounding_box_score ~ in_treatment+as.factor(didbf))]
e3_mod_4 <- d[dataset_no==3, lm(bounding_box_score ~ in_treatment+as.factor(age))]
e3_mod_5 <- d[dataset_no==3, lm(bounding_box_score ~ in_treatment+as.factor(edu))]
e3_mod_6 <- d[dataset_no==3, lm(bounding_box_score ~ in_treatment+as.factor(income))]
...
e3_mod_7 <- d[dataset_no==3, lm(bounding_box_score ~ in_treatment+(monitor=="cellphone"))]
...
e4_mod_1 <- d[dataset_no %in% c(3,4), lm(bounding_box_score ~ in_treatment)]
...
e4_mod_2 <- d[dataset_no %in% c(3,4), lm(bounding_box_score ~ in_treatment+is_mobile)]

stargazer(e4_mod_1, e4_mod_2,  
          type=stargazer_type, header = FALSE, table.placement = 'h', report=('vc*p'),
          add.lines = list(c("Data Subset", "All", "All", "$x==1$")),
          column.labels=c("Target Alone", "Controlling for Mobile"))
...
e4_ate = d[dataset_no %in% c(3, 4) & in_treatment == 1, mean(bounding_box_score, na.rm=T)] - d[dataset_no %in% c(3, 4) & in_treatment == 0, mean(bounding_box_score, na.rm=T)]

e4_sd = d[dataset_no %in% c(3, 4), sd(bounding_box_score, na.rm=T)]
...
```{r experiment_4_more_data, echo=FALSE, results='asis'}


e4_mod_3 <- d[dataset_no %in% c(3,4,7), lm(bounding_box_score ~ in_treatment+is_mobile)]

stargazer(e4_mod_3, e4_mod_2,  
          type=stargazer_type, header = FALSE, table.placement = 'h', report=('vc*p'),
          add.lines = list(c("Data Subset", "All", "All", "$x==1$")),
          column.labels=c("Target Alone", "Controlling for Mobile"))
```
...
e5_mod_1 <- d[dataset_no == 5, lm(bounding_box_score ~ in_treatment)]
summary(e5_mod_1)
```
...
e6_mod_1 <- d[dataset_no == 6, lm(bounding_box_score ~ in_treatment)]
summary(e6_mod_1)
```
...
e7_mod_1 <- d[dataset_no %in% c(3,4,7), lm(bounding_box_score ~ in_treatment)]
e7_mod_2 <- d[dataset_no %in% c(3,4,7), lm(bounding_box_score ~ in_treatment+is_cellphone)]
e7_mod_3 <- d[dataset_no %in% c(3,4,7), lm(bounding_box_score ~ in_treatment+factor(mousetrackpad))]
e7_mod_4 <- d[dataset_no %in% c(3,4,7), lm(bounding_box_score ~ in_treatment+factor(Reward))]
e7_mod_5 <- d[dataset_no %in% c(3,4,7), lm(bounding_box_score ~ in_treatment+factor(mousetrackpad)+factor(Reward))]

stargazer(e4_mod_1, e4_mod_2, e7_mod_1, e7_mod_2, e7_mod_3,
          type=stargazer_type, header = FALSE, table.placement = 'h', report=('vc*p'),
          add.lines = list(c("Data Subset", "All", "All", "$x==1$")),
