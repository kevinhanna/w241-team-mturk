...
get_result_data_all_experiments <- function() {
  ...
  rbind(mturk_results_dt_1, mturk_results_dt_2)
}
...
extract_image_bounding_data <- function(dt) {
  ...
  for (i in 1:nrow(dt)) {
    ...
    bounding_boxes_dt[, cellphone := row[, as.numeric(Answer._2monitor1.cellphone)]]
    bounding_boxes_dt[, tablet := row[, as.numeric(Answer._2monitor2.tablet)]] 
    bounding_boxes_dt[, smalllaptop := row[, as.numeric(Answer._2monitor3.smalllaptop)]]    
    bounding_boxes_dt[, midsize := row[, as.numeric(Answer._2monitor4.midsize)]]
    bounding_boxes_dt[, largescreen := row[, as.numeric(Answer._2monitor5.largescreen)]]    
    bounding_boxes_dt[, notsure := row[, as.numeric(Answer._2monitor6.notsure)]]
    
    
    
    bounding_boxes_dt$q01.didbf<-ifelse(row$Answer._1donebf1.yes ==TRUE, "yes", 
                     ifelse(row$Answer._1donebf2.no == TRUE, "no",
                     ifelse(row$Answer._1donebf3.dontknow == TRUE, "dontknow","noans")))
    bounding_boxes_dt$q02.monitor<-  ifelse(row$Answer._2monitor1.cellphone   ==TRUE, "cellphone", 
                     ...
                     ifelse(row$Answer._2monitor6.notsure == TRUE, "notsure","noans"))))))
    bounding_boxes_dt$q03.mousetrackpad<-ifelse(row$Answer._3cursor1.mouse == TRUE, "mouse", 
                     ifelse(row$Answer._3cursor2.trackpad == TRUE, "trackpad",
                     ifelse(row$Answer._3cursor3.other == TRUE, "other","noans")))
    bounding_boxes_dt$q04.age<-ifelse(row$Answer._4age1.lt21 ==TRUE, "lt21", 
            ifelse(row$Answer._4age2.21to30 == TRUE, "21to30",
            ifelse(row$Answer._4age3.31to30 == TRUE, "31to40", 
                   #31 to 30 is coded wrong, but no impact
            ifelse(row$Answer._4age4.41to50 == TRUE, "4150",
            ifelse(row$Answer._4age5.over50r == TRUE, "over50","noans")))))
    bounding_boxes_dt$q05.income<-ifelse(row$Answer._5income1.lt10k ==TRUE, "lt10k", 
               ...
               ifelse(row$Answer._5income5.gt90k == TRUE, "gt90k","noans")))))
    bounding_boxes_dt$q06.edu<-ifelse(row$Answer._6edu1.lthighschool ==TRUE, "lthighschool", 
            ...
            ifelse(row$Answer._6edu5.masterorabove == TRUE, "masterorabove","noans")))))
...
formatted_mturk_image_boundings_dt[, bounding_box_score := score_mturk_results(formatted_mturk_image_boundings_dt, formatted_mturk_image_boundings_row)]
