...
get_result_data_experiment_5 <- function() {
  control <- fread("../data/experiment_5/Batch_3854419_batch_results_norelation_1qn.csv")
  control[, in_treatment := 0]
  
  treatment <- fread("../data/experiment_5/Batch_3853897_batch_results_oneqn_relationship.csv")
  treatment[, in_treatment := 1]
...
extract_image_bounding_data <- function(dt) {
  ...
  for (i in 1:nrow(dt)) {
    ...
    bounding_boxes_dt$age<-ifelse(row$Answer._4age1.lt21 ==TRUE, "lt21", 
    ...
    bounding_boxes_dt$is_mobile <- as.numeric((row$Answer._2monitor1.cellphone | row$Answer._2monitor2.tablet))
    bounding_boxes_dt$is_cellphone <- as.numeric(row$Answer._2monitor1.cellphone)
